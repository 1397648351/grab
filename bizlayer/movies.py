    ...
    def get_movie(self, url):
        try:
            ...
            html = Grab.get_content(url).replace('xmlns="http://www.w3.org/1999/xhtml" /', '').replace(
                'xmlns="http://www.w3.org/1999/xhtml"', '').decode('gb2312', 'ignore')
            doc = pq(html)
            info['download'] = doc('#Zoom table a:first-child').text().strip()
            ...
            pattern = re.compile(ur'.*◎.*评分(?P<score>.*)/10 from .* users.*')
            res = re.match(pattern, content)
            if res:
...
if __name__ == "__main__":
    m = Movies()
    m.run(6, 10)
    # m.get_movie('https://www.dytt8.net/html/gndy/dyzz/20180913/57447.html')
